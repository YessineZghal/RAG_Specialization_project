"""Shared fixtures for Level 8's offline test suite -- same fake-LLM/
...
No Ollama required -- verifies orchestration logic (does pruning actually
stop exploring a low-scored branch? does the graph aggregation step
actually combine two nodes?) against scripted fakes; "does this work
against a real model" is left to the notebooks, which do hit the real,
running Ollama.
"""
...
class FakeRetriever:
    """Duck-types `reasoning_common.retrieval.DenseRetriever`'s `.search()`
    with a scripted result list, ignoring the actual query text.
    """
